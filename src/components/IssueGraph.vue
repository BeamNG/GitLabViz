...
// Re-emit menu mutations so App.vue's existing handlers keep working.
...
// State for the floating issue context menu (visibility + position passed to
// the shared IssueContextMenu component). `selectedNodeId` drives the focus
// outline on the canvas while the menu is open.
...
function openContextMenu (event, node) {
  // Viewport coords — IssueContextMenu uses position: fixed and clamps onto screen.
...
// Close the legend color popover when clicking outside the legend. The issue
// context menu handles its own outside-click close inside IssueContextMenu.
const onGlobalMouseDown = (e) => {
  if (!legendColorPopover.value.visible) return
  const legend = legendEl.value
  const pop = legendColorPopoverEl.value
  const t = e && e.target ? e.target : null
  if (t && legend && legend.contains(t)) return

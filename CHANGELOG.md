...
## [0.13.8] - 2026-07-24
- **Flake History: a test that fails and then passes on the serial retry now reads as flaky rather than broken.** CI re-runs unmatched failures once, serially, and a test that passes on that second attempt is counted as passed by the suite — but flake history still recorded only the original failure, so a test that flaked and recovered on *every* run had no passes at all and was rendered as broken. Such a run now shows as an **amber** heatmap cell (dark orange once its artifacts expire, matching the existing pass/fail convention), the leaderboard labels the test *actively flaky* instead of *broken*, and the build-health card counts the run as passed so the days-without-failures streak keeps counting — which is what the suite's own green exit code already said. A failure that stayed red is unaffected.
- **The pass rate deliberately stays the first-attempt rate.** A test whose failures all recovered still reads 0%, and still sorts to the top of the leaderboard — the retry is a mitigation, not a pass, and hiding the flake would defeat the point of tracking it. Only the label and the colour change.
- **The bundle schema gains optional `flaky_run_ids` and `flaky_count` per context, and `last_status` may now be `"flaky"`.** `flaky_run_ids` is always a subset of `failing_run_ids`. The schema stays at version 1 and the fields are optional in both directions: a bundle published before this change renders exactly as it does today, and an older viewer reading a newer bundle simply ignores the new keys.
- **The "Flake classifications" help dialog no longer contradicts itself.** It described *broken* as "zero passes observed" and *actively flaky* as "pass rate < 50%", which now describe the same row — a recovered-only test reads 0% and is labelled actively flaky. *Broken* now reads as never passed **and** never recovered on the retry; *actively flaky* notes that the pass rate can still read 0%.

## [0.13.7] - 2026-07-16
- **Flake History heatmap: pipeline columns now read dx12 → vulkan → dx11, with a separator between pipelines.** Within each CI pipeline the graphics-backend columns are ordered `dx12`, `vulkan`, `dx11` (matching the order the CI notifier posts its lanes in), and a thin vertical line is drawn before the first column of each pipeline so the trios are easy to tell apart at a glance. Runs with no pipeline id are treated as their own single-column groups, as before.
- **Flake History: right-click a heatmap cell → "Show Revision Details".** Opens that run's revision on your VCS host in a new tab. The URL is built from a new **Commit view base URL** setting (see below) with the run's revision appended; the item is disabled when the run has no revision or the base URL hasn't been configured yet.
- **Flake History: right-click a heatmap cell → "Copy Revision #".** Copies the plain revision number to the clipboard (best-effort, with a fallback for non-secure contexts) so you can paste it straight into your SVN client's revision filter. Disabled only when the run has no revision.
- **New "Commit view base URL" setting** (`commitViewBaseURL`, in the Flake History settings dialog). The per-commit URL base for your VCS host — the revision id is appended to it (one slash is inserted if the base lacks a trailing one). It ships as a neutral placeholder (`https://<repository_provider>/<group>/<project>/-/commit/`, following the GitLab/GitHub commit-URL shape) that you override for your environment; until it's set to a real value, "Show Revision Details" stays disabled.

## [0.13.6] - 2026-06-24
- **Flake History leaderboard: falls back to the stable roster when nothing is flaky.** In flaky-only mode, a window with no flakes used to leave the leaderboard blank; it now shows the full stable test list with a "no flakes in window; showing all stable tests" caption, so a healthy suite reads as "all green" instead of "no data".
- **Flake History heatmap: columns grouped and ordered by CI pipeline.** Runs are grouped by `pipeline_id` and ordered by pipeline number so each pipeline's runs sit contiguously (rather than interleaving by timestamp), and the window is measured in whole pipelines rather than individual runs — so a trio is never clipped mid-group.
- **Flake History: the window slider now counts pipelines.** The "Last N" slider windows by number of pipelines (default 10) to match the pipeline-grouped columns.

## [0.13.5] - 2026-06-17
- **Flake History heatmap: each column header now shows its pipeline number under the date.** The vertical run-column headers used to show only the run date (`MM-DD`); they now stack a dimmer `#<pipeline_id>` line beneath it, so it's easy to tell at a glance which columns belong to the same CI pipeline (i.e. which runs went together). Runs with no pipeline id just show the date, unchanged.
- **Flake History: the heatmap cell tooltip now shows the revision, suite and gfx target.** Hovering a cell already listed the test name, pass/fail, the run hash and artifact availability; it now appends a context line `r<source_revision> · <suite> · <gfx_api>` (e.g. `r175518 · nightly · dx12`) so you can see which VCS revision the run was built against and which suite flavour / graphics backend it was, without leaving the heatmap. Missing values fall back to `?`, mirroring the column-header tooltip.
- **Flake History: filter by a revision range straight from the search box.** The existing "Search tests…" box now doubles as a revision filter — type `r177518...r177600` to narrow **both** the leaderboard and the heatmap to runs built against revisions in that range. Open-ended (`r177518...`, `...r177600`) and single-revision (`r177518`) forms are supported, as is a `…` ellipsis (handy when the OS autocorrects `...`). The `...`/`…` marker distinguishes a range from a name search; a bare single revision needs the leading `r` so an ordinary numeric search like `800` is left alone. Runs whose revision is missing or non-numeric are excluded while a range is active, and a range matching nothing shows a "No runs match this revision range" hint. Anything that isn't a revision expression still searches test names as before.

## [0.13.4] - 2026-06-03
- **Flake History: open the local viewer via a custom scheme command instead of a file path.** A new **"Use command listener trigger"** setting (in the Flake History settings dialog and the first-run form) switches how a heatmap click opens the viewer: rather than opening a local file (`shell.openPath` / `file://`), GitLabViz fires a configurable scheme URL that your local tooling can handle. The command is set by **"Command listener call"** (default `command:v1/run_custom_command`, a neutral placeholder you override for your environment) and is fired verbatim — in the desktop build through the OS handler (`shell.openExternal`), in the browser through a best-effort anchor-click. The existing file-based open stays the default (toggle off), so nothing changes unless you opt in.
- **New "open viewer on click" toggle in the app bar.** A per-session switch (top-right, monitor icon) controls whether a left-click also opens the viewer at all. It defaults **on** every time the page loads and is intentionally **not** persisted — when off, clicking a cell only downloads the artifacts (or opens the pipeline once they've expired) without surfacing the viewer.
- **Right-click context menu on heatmap cells.** Right-clicking a cell now offers **Download artifact** (disabled once artifacts have expired or when none was published) and **Go to pipeline** (disabled when the run has no pipeline URL) — the same two actions a left-click chooses between, now available explicitly.
...
- **New Flake History view + kiosk mode + public bundle schema.** GitLabViz can now read a `flake-history-bundle.v1.json` published by any CI source and render two new screens for it:
  ...
  - A **kiosk panel** (`flake` mode, opt-in) tuned for legibility-at-distance: "Currently broken" + "Top flaky" lists with 5-run dot sparklines on the flaky side.
  - A **public producer contract** under `docs/flake-history-bundle.v1.schema.json` (JSON Schema draft-2020-12) with a worked `fixtures/flake-history-bundle-sample.json` and `docs/flake-history-integration.md` explaining how to publish a bundle from any CI — `pipeline_url` is optional, `additionalProperties: true` everywhere so producers can ship experimental fields without breaking validation. The schema is the only surface the viewer commits to.
- Settings live under **Configuration → Flake History** (`projectId`, `packageName`, `refreshMinutes`); URL + token reuse the existing GitLab settings. First-time users get an inline configuration form inside the view.
- **Fix:** `#/flake` is now a real, bookmarkable hash route. Previously `useHashRouting` only knew `main / config / chattools / kiosk`, so clicking the Flake sidebar button briefly rendered the view and then the router resynced `activePage` back to `main` — the screen flashed and bounced back to "No data loaded". Two regression tests pin the no-bounce behavior and the inbound-hash sync.
- **UX polish on the new view:** split toolbar (primary app-bar with back / title / search / theme / info / refresh, secondary filter row), heatmap cells redesigned as padded tiles with hover scale + soft shadow so they read as clickable instead of a fused band, leaderboard and heatmap each in their own rounded surface card, theme toggle reuses the global light → dark → system → schedule cycle, data table paginates at 50/page, redundant checkbox column dropped (row click already selects, heatmap cross-highlight unchanged).
- **Note for producers:** the four classification thresholds in `src/services/flake.js → classifyFromCounts()` are kept in sync with `bundle_flake_history.py → _flake_classification` on the producer side. If you publish your own bundles, mirror those thresholds or your "broken" labels will disagree with the viewer's chips.
